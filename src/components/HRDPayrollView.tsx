      ...
      <div className="rounded-xl border border-cyan-500/30 bg-gradient-to-r from-slate-900 via-cyan-950/40 to-slate-900 p-6 text-white shadow-xl">
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          ...
          <div className="flex items-center gap-3">
            <button
              onClick={() => {
                const name = prompt('Nama Karyawan / Dokter Baru:');
                if (name) {
                  const emp: EmployeeRecord = {
                    id: `emp-${Date.now()}`,
                    employeeNip: `NIP-${Math.floor(10000000 + Math.random() * 90000000)}`,
                    name,
                    role: 'Dokter Umum',
                    department: 'Poliklinik Umum',
                    strNumber: '31.1.2.300.4.20.99881',
                    strExpiryDate: '2029-01-01',
                    sipNumber: '503/123/SIP/2024',
                    sipExpiryDate: '2029-01-01',
                    employmentStatus: 'Tetap',
                    basicSalary: 8500000,
                    medicalFeeShare: 5000000,
                    kpiScore: 90,
              ...
              className="flex items-center gap-2 rounded-lg bg-cyan-500 px-4 py-2.5 text-sm font-semibold text-slate-950 hover:bg-cyan-400 shadow-lg transition"
            >
              <Plus className="h-4 w-4" />
              Tambah Karyawan / Dokter
            </button>
          </div>
        </div>
      </div>
